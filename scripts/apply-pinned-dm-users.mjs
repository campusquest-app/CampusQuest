#!/usr/bin/env node
/**
 * Apply supabase/migrations/20260623130000_pinned_dm_users.sql
 *
...
function migrationPath() {
  return resolve(root, "supabase/migrations/20260623130000_pinned_dm_users.sql");
}
...
async function verifyWithSupabaseJs() {
  ...
  const { error } = await admin.from("pinned_dm_users").select("id").limit(1);
  if (error) {
    console.error("Verification failed:", error.message);
    return false;
  }
  console.log("Verified: public.pinned_dm_users table exists.");
  return true;
}
...
async function applyWithPg(sql) {
  ...
  try {
    ...
    console.log("Applied pinned_dm_users migration successfully.");
    return true;
  } finally {
    await client.end();
  }
...
async function main() {
  ...
  if (ok) {
    console.log("pinned_dm_users table already exists — no SUPABASE_DB_URL needed.");
    process.exit(0);
  }

  console.error(
    "\npinned_dm_users table is missing. Run the migration SQL in Supabase Dashboard, then restart `npm run dev`.",
  );
  process.exit(1);
}
