#!/usr/bin/env node
/**
 * Apply supabase/migrations/20260618120000_dm_rich_messages.sql
 *
...
function migrationPath() {
  return resolve(root, "supabase/migrations/20260618120000_dm_rich_messages.sql");
}
...
async function verifyWithSupabaseJs() {
  ...
  const { error } = await admin.from("direct_messages").select("type").limit(1);
  if (error) {
    console.error("Verification failed:", error.message);
    return false;
  }
  console.log("Verified: direct_messages.type column exists.");
  return true;
}
...
async function applyWithPg(sql) {
  ...
  try {
    ...
    console.log("Applied dm_rich_messages migration successfully.");
    return true;
  } finally {
    await client.end();
  }
...
async function main() {
  ...
  if (ok) {
    console.log("DM rich message columns already exist — no SUPABASE_DB_URL needed.");
    process.exit(0);
  }

  console.error(
    "\nDM rich message columns are missing. Run the migration SQL in Supabase Dashboard, then restart `npm run dev`.",
  );
  process.exit(1);
}
