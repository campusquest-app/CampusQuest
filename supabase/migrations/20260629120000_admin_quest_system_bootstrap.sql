-- Idempotent bootstrap for admin quest system (tables, indexes, RLS).
-- Safe to run on remote DB that never received 20260625120000 / 20260628120000.
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'admin_quests_qr_code_id_fkey'
  ) then
    alter table public.admin_quests
      add constraint admin_quests_qr_code_id_fkey
...
create index if not exists idx_admin_quests_visibility
  on public.admin_quests(visibility_status, starts_at, ends_at);

create index if not exists idx_admin_quests_created
  on public.admin_quests(created_at desc);

create index if not exists idx_admin_quests_location
  on public.admin_quests(location_lat, location_lng)
  where location_lat is not null and location_lng is not null;

create index if not exists idx_admin_quests_location_key
  on public.admin_quests(location_key)
...
create unique index if not exists idx_admin_quest_completions_unique_day
  on public.admin_quest_completions(quest_id, user_id, completion_day)
  where completion_day is not null;

create index if not exists idx_admin_quest_completions_quest
  on public.admin_quest_completions(quest_id, completed_at desc);

create index if not exists idx_admin_quest_completions_user
  on public.admin_quest_completions(user_id, completed_at desc);
...
create index if not exists idx_quest_templates_category
  on public.quest_templates(category, usage_count desc);

alter table public.qr_codes
  add column if not exists admin_quest_id uuid,
  add column if not exists qr_type text default 'general',
  add column if not exists location_key text,
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'qr_codes_admin_quest_id_fkey'
  ) then
    alter table public.qr_codes
      add constraint qr_codes_admin_quest_id_fkey
      foreign key (admin_quest_id) references public.admin_quests(id) on delete set null;
  end if;
exception
  when undefined_table then null;
end $$;

create index if not exists idx_qr_codes_admin_quest
  on public.qr_codes(admin_quest_id)
  where admin_quest_id is not null;
...
drop policy if exists "admin_quests admin all" on public.admin_quests;
create policy "admin_quests admin all"
on public.admin_quests for all
to authenticated
using (
  exists (
...
drop policy if exists "admin_quest_completions admin all" on public.admin_quest_completions;
create policy "admin_quest_completions admin all"
on public.admin_quest_completions for all
to authenticated
using (
  exists (
    select 1 from public.profiles p
...
drop policy if exists "quest_templates admin all" on public.quest_templates;
create policy "quest_templates admin all"
