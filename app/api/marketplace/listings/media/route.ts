...
export async function POST(request: Request) {
  try {
    ...
    enforceRateLimit({ userId: auth.user.id, routeKey: "marketplace:media:upload", limit: 30, windowMs: 60_000 });
    ...
    const mime =
      normalizeImageMime(file.type || "") || sniffImageMimeFromBuffer(buffer, fileName) || "";
