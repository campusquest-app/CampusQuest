...
import { resetPlacementToAutoMatch } from "@/lib/server/externalEventMapOverrides";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    enforceRateLimit({
      userId: auth.user.id,
      routeKey: "admin:urinvolved-placement:reset",
      limit: 30,
      windowMs: 60_000,
    });
    ...
    const override = await resetPlacementToAutoMatch({ externalEventId, fields, catalog });
    return ok({ override });
  } catch (error) {
