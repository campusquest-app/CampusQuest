...
export async function POST(request: Request) {
  try {
    ...
    enforceRateLimit({ userId: auth.user.id, routeKey: "internal:moderation:campus:content:post", limit: 20, windowMs: 60_000 });
    const input = await readJson(request, moderateCampusContentSchema);

    const response = await fetch(new URL("/api/moderation/campus/content", request.url).toString(), {
      method: "POST",
      headers: {
