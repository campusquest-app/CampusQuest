...
export async function GET(request: Request) {
  try {
    ...
    enforceRateLimit({ userId: auth.user.id, routeKey: "internal:moderation:appeals:get", limit: 30, windowMs: 60_000 });
    const url = new URL("/api/moderation/appeals", request.url);
...
export async function POST(request: Request) {
  try {
    const auth = await requireAdminUser(request as any);
    enforceRateLimit({ userId: auth.user.id, routeKey: "internal:moderation:appeals:post", limit: 20, windowMs: 60_000 });
