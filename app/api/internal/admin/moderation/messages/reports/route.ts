...
async function forwardModerationRequest(request: Request, init: RequestInit) {
  const url = new URL("/api/moderation/messages/reports", request.url);
...
export async function GET(request: Request) {
  try {
    ...
    enforceRateLimit({ userId: auth.user.id, routeKey: "internal:moderation:reports:get", limit: 30, windowMs: 60_000 });
...
export async function POST(request: Request) {
  try {
    const auth = await requireAdminUser(request as any);
    enforceRateLimit({ userId: auth.user.id, routeKey: "internal:moderation:reports:post", limit: 20, windowMs: 60_000 });
